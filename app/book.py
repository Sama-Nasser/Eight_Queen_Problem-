...
# Tabs
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📊 General Analysis", "🏆 Best & Worst Books", "📦 Availability & Description", "☁ Word Cloud", "📖 Book Details", "📑 Book Analytics"])
...
with tab2:
    ...
    if ax4.get_legend() is not None:
        ax4.get_legend().set_visible(False)  # Remove legend
    plt.xticks(rotation=90)
    st.pyplot(fig4)
...
with tab3:
    ...
    if ax5.get_legend() is not None:
        ax5.get_legend().set_visible(False)  # Remove legend
    st.pyplot(fig5)
...
# ----- ☁ Word Cloud -----
with tab4:
    st.header("☁ Word Cloud from Book Descriptions")
